...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    div.innerHTML = `<div class="single-product">
      ...
      <button onclick="addDetails('${product.description}')" id="details-btn" class="btn btn-success">Details </button> 
      <div id="ratingBox">
      <div id="ratingGrid">
      ...
      <div>
        <p class="ratingText" >Customer Choice:${product.rating.rate} </p>
        <p class="ratingText" >Customer Number: ${product.rating.count}</p>
      </div> 
    </div>
      `;
...
const addDetails = (id)=>{
  ...
  if(detailItem.style.display='none'){
      showBlock(); 

    if(detailItem.textContent){
      emptyDetail();  
    }
    if(!detailItem.textContent){
      const div = document.createElement('div'); 
...
const getInputValue = (idPlace) => {
  const element = document.getElementById(idPlace).innerText;
  const elementNumber = Number(element); 
  const converted = parseFloat(elementNumber);
  return converted;
};
